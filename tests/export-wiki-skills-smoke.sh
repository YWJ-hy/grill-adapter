...
# --- A standalone wiki repo: content at the repo root, no .grill-adapter/wiki nesting ---
...
printf '\nTest: graph-rebuild CI is stamped by default\n'
WORKFLOW="$TMP/.github/workflows/rebuild-wiki-graph.yml"
assert_file_exists "workflow emitted" "$WORKFLOW"
assert_contains "export reports graph CI" "graph CI: .github/workflows/rebuild-wiki-graph.yml" "$EXPORT_OUT"
WORKFLOW_CONTENT="$(cat "$WORKFLOW")"
assert_contains "workflow carries adapter marker" "Generated by grill-adapter" "$WORKFLOW_CONTENT"
assert_contains "workflow reuses vendored helper" "wiki-tools/scripts/wiki_migrate_helper.py" "$WORKFLOW_CONTENT"
assert_contains "workflow has skip-ci loop guard" "[skip ci]" "$WORKFLOW_CONTENT"
if [[ "$WORKFLOW_CONTENT" != *"__ADAPTER_GENERATED_MARKER__"* ]]; then
  printf '  ✓ marker placeholder was substituted\n'; PASS=$((PASS + 1))
else
  printf '  ✗ marker placeholder was substituted (still literal)\n'; FAIL=$((FAIL + 1))
fi
MANIFEST_CONTENT="$(cat "$TMP/.claude/wiki-tools/.export-manifest.json")"
assert_contains "manifest records graphCi" '"graphCi": true' "$MANIFEST_CONTENT"
assert_contains "manifest hashes the workflow" ".github/workflows/rebuild-wiki-graph.yml" "$MANIFEST_CONTENT"
...
printf '\nTest: neutrality guard enforced from .grill-adapter/settings.json\n'
mkdir -p "$TMP/.grill-adapter"
cat > "$TMP/.grill-adapter/settings.json" << 'JSON'
{
  "wiki": {
    "roots": {
      "shared": {
        "sharedNeutrality": {
          "blockedTerms": ["AcmeCorpInternal"]
        }
      }
    }
  }
...
printf '\nTest: --no-graph-ci removes the managed workflow (real off switch)\n'
NOCI_OUT="$("$ADAPTER_ROOT/manage.sh" export-wiki-skills "$TMP" --no-graph-ci 2>&1)"
assert_contains "export reports CI disabled" "graph CI: disabled" "$NOCI_OUT"
if [[ ! -f "$WORKFLOW" ]]; then
  printf '  ✓ managed workflow removed\n'; PASS=$((PASS + 1))
else
  printf '  ✗ managed workflow removed (still present)\n'; FAIL=$((FAIL + 1))
fi
NOCI_MANIFEST="$(cat "$TMP/.claude/wiki-tools/.export-manifest.json")"
assert_contains "manifest records graphCi false" '"graphCi": false' "$NOCI_MANIFEST"

printf '\nTest: an unmanaged workflow is left untouched by --no-graph-ci\n'
mkdir -p "$TMP/.github/workflows"
echo "name: my own workflow" > "$WORKFLOW"
"$ADAPTER_ROOT/manage.sh" export-wiki-skills "$TMP" --no-graph-ci >/dev/null 2>&1 || true
assert_file_exists "hand-authored workflow preserved" "$WORKFLOW"
assert_contains "hand-authored workflow unchanged" "my own workflow" "$(cat "$WORKFLOW")"
rm -f "$WORKFLOW"
