...
before="$(cd "${PROJECT_ROOT}" && { find .grill-adapter/wiki .shared-adapter/wiki -type f -name '*.md' -print0 2>/dev/null || true; } | xargs -0 shasum 2>/dev/null || true)"
json_output="$(cd "${PROJECT_ROOT}" && python3 "${TARGET_INPUT}/scripts/init-wiki.py" . "self-test" --json)"
after="$(cd "${PROJECT_ROOT}" && { find .grill-adapter/wiki .shared-adapter/wiki -type f -name '*.md' -print0 2>/dev/null || true; } | xargs -0 shasum 2>/dev/null || true)"
