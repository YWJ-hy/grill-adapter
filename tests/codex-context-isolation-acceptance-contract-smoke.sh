...
grep -Fq 'ISSUE44_EXPLICIT_ACTIVATION' "$ACCEPTANCE"
grep -Fq 'ISSUE44_HOST_MARKER_ACTIVATION' "$ACCEPTANCE"
grep -Fq 'ISSUE44_UNWIRED_STANDALONE' "$ACCEPTANCE"
grep -Fq 'project_activation.py preflight with --explicit' "$ACCEPTANCE"
grep -Fq 'host marker and no .grill-adapter/settings.json' "$ACCEPTANCE"
grep -Fq 'no AGENTS.md or CLAUDE.md host marker' "$ACCEPTANCE"
grep -Fq 'completed_parent_count' "$ACCEPTANCE"
