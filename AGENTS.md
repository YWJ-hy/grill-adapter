...
先读 [`docs/DEVELOPMENT_CN.md`](docs/DEVELOPMENT_CN.md) 的变更路由矩阵。它按变更类型指向最小权威文档和验证命令；普通单子系统修改不再强制读取完整 architecture、host、user-flow、Obsidian 运维和历史蓝图。再按矩阵读取相关 `skills/*/SKILL.md`、`agents/*.md`、`contracts/` 或 `host-adapters/*/AGENTS.md`。

最终验收以 Codex 集成路径为准：真跑 `grill-with-docs → to-spec/to-tickets → implement → code-review → update-wiki`，不能只以直接执行 Python 脚本成功为准。
...
bash tests/codex-plugin-smoke.sh "$PWD"
...
- 改用户可见行为时，按 workflow 路由同步检查/更新 `docs/USER_FLOW_CN.md`、对应 skill/agent、`host-adapters/*/AGENTS.md` 和 `README.md`。
...
- 改插件布局时运行 `codex-plugin-smoke.sh` 与 `documentation-index-smoke.sh`，并同步 `release-check.sh` 的组件计数。
