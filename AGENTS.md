...
This file provides guidance to Codex (Codex.ai/code) when working with code in **this repository** (developing grill-adapter itself). It is not the convention block that gets installed into a target project — that lives under `host-adapters/`.
...
4. 与当前改动相关的 `skills/*/SKILL.md` / `host-adapters/*/AGENTS.md`。

**验收铁律：最终验收以 Codex 集成路径为准**——真跑 `(可选 lanhu-requirements) → grill-with-docs → to-spec/to-tickets → implement → code-review → update-wiki`，不能只以直接执行 Python 脚本成功为准。

## 常用命令

```bash
# 插件本体（skills/agents/hooks/MCP 全在里面，一起激活）
Codex plugin install grill-adapter@grill-adapter --scope project   # 或 --scope user
Codex --plugin-dir "$PWD" plugin details grill-adapter             # 开发期：不装也能加载 + 看组件清单

# 项目接线（插件唯一做不到的事：改目标项目的 AGENTS.md）
...
本仓库是 grill-adapter 的**源码**，不是业务项目代码。它**本身就是一个 Codex 插件**：`.Codex-plugin/plugin.json` 声明包，Codex 自动发现 `skills/`、`agents/`、`hooks/hooks.json`、`.mcp.json`。通过独立 skill/agent/hook + 项目 AGENTS.md 约定增强宿主工作流，**绝不 patch 宿主 skill**。

三层：

- **Host 适配器（薄、可插拔、零 skill patch）**：`host-adapters/grill/AGENTS.md`（默认宿主 grill：约定块）、`host-adapters/plain/AGENTS.md`（裸 Codex）。`install.py` 把选定约定块写进目标 `AGENTS.md`——这是项目级唯一动作。约定块**只点名 skill、不含任何安装路径**：它落在目标项目里（非插件内容），`${CLAUDE_PLUGIN_ROOT}` 在那里不会被替换，而烤死绝对路径会在插件升级后腐烂。
...
- **引擎（从旧 adapter 原样移植）**：`scripts/*.py`（wiki + source_truth + lanhu 执行层）、`scripts/grill_context_to_candidates.py`（grill→wiki 桥，由 `skills/update-wiki` 调用）、`mcp/shared-wiki/`（shared-wiki MCP，读 `CLAUDE_PROJECT_DIR` 自配置）、`.graph.json`（派生物）、`wiki-template/`、`wiki-repo-skills/`、`wiki-repo-ci/`、`contracts/`。
...
- 改用户可见行为时，同步检查/更新 `docs/USER_FLOW_CN.md`、相关 skill、`host-adapters/*/AGENTS.md` 与 `README.md`。
...
- 改插件布局（新增/删除 skill、agent、hook、MCP）后，跑 `Codex --plugin-dir "$PWD" plugin details grill-adapter` 核对组件计数，并同步 `release-check.sh` 步骤 6 里写死的期望值。
