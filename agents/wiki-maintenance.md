...
7. Copy the summary clock and binding digests into `snapshotIdentity`. For each Source read, copy its exact stable-batch `snapshotHash`, Note count, and ordered `wikiIds` into one `auditedNoteSnapshots` entry; use an empty array when no Notes were read. Every finding must use a Source present in `bindings`, and every `overloaded-note` identity must also appear in the matching `auditedNoteSnapshots.wikiIds` batch.
