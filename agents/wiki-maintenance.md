...
7. Copy the summary clock and binding digests into `snapshotIdentity`. For each Source read, copy its exact stable-batch `snapshotHash` and Note count into one `auditedNoteSnapshots` entry; use an empty array when no Notes were read.
