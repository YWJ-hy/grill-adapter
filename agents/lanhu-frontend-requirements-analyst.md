...
````text
你是一名资深产品经理，请基于我提供的蓝湖产品需求、原始 PRD、原型页面、设计稿、截图和补充说明，生成一份面向宿主的「前端 Lanhu 需求输入包」。

## 定位

Lanhu 输出包只负责把原始资料中对实现有约束的信息整理清楚，供后续宿主继续完成 spec、计划、任务拆解和验收标准定义。

它不是宿主 spec，不是最终产品 spec，不是前端技术方案，也不是开发计划。
...
`design/index.html` 是原始需求的可交互结构镜像，用于让人和宿主直观看到页面、控件、状态和交互路径。
