...
| **Capture** | every stage appends to one feature journal; evidence-backed corrections retain an affected `sourceId` + stable `wikiId` as unresolved maintenance signals, ADR increments become metadata-only execution-projection candidates, and `scaffold-practice-skill` stages content-addressed Card candidates. `/grill-adapter:update-wiki` reconciles final evidence, requires exact bound identity plus a matching governed update before keeping a correction, makes explicit atomic-Note target decisions, and applies policy-compliant Note/Card changes. Publishing retained applied receipts is an explicit later action that creates resumable per-repository draft PRs; open PRs remain pending | after `/code-review` |
