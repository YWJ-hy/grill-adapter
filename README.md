...
| **Bind** | Planning freezes all roster tasks' schema-v2 `<taskId>.wiki-implement.md` and `<taskId>.wiki-review.md` from the same approved Source snapshot in one batch; the high-level `/grill-adapter:wiki-readiness` entry resolves one task identity, validates fingerprints and both role contracts, atomically commits direct/manual late Carry after routing approval, reuses or compatibility-freezes missing snapshots, and derives `<taskId>.wiki-review-handoff.md` without overwriting the approved reviewer snapshot | `/to-tickets` approval + `/implement` + before `/code-review` subagents |
...
freshness, contradiction, and overloaded-Note judgment to the dedicated read-only
`wiki-maintenance-audit` role. `wiki-maintenance consolidation <feature-slug>` separately loads
`wiki-maintenance-consolidation`: a dedicated read-only MCP operation deterministically replays unresolved
...
Both runtimes discover **12 skills, 5 agents, 3 hook events, and 1 MCP server**. `wiki-readiness` is the single host-facing Bind entry; its materialize reader is internal and does not occupy the skill catalog. Claude Code registers `wiki-researcher`, `wiki-capture`, `wiki-maintenance-audit`, `wiki-maintenance-consolidation`, and `wiki-outbox-consolidation` directly. For research, Capture, and each maintenance mode, the Codex coordinator resolves only a trusted role descriptor and dispatches one general sub-agent with no inherited conversation; that child verifies the role source and digest before loading exactly its current role and calling Wiki tools. The returned agent path is only a handle: waiting on that exact path is the only permitted next operation, and the main session must repeat bounded waits until terminal before messaging the user, asking questions, calling MCP, or continuing. Dispatch/capacity/lifecycle or role-loading failures are classified as `broken` rather than an empty success. The default `obsidian-wiki` MCP surface excludes legacy proposal/apply and publishes shallow envelopes for Capture Plan and Outbox correction while deterministic execution retains full validation. Caller-scoped surfaces are available with `--profile research|capture|maintenance-audit|maintenance-consolidation|outbox|legacy`; `migration` aliases `legacy`. `SessionStart` emits at most one counts-only queued-work reminder. Governed proposal/apply and the loopback write bridge remain for migration and legacy recovery; normal Capture never invokes them or edits formal base. Use `$grill-adapter:setup-init-obsidian` (or `/grill-adapter:setup-init-obsidian`) to initialize a project (details: [`docs/OBSIDIAN_WIKI_CN.md`](docs/OBSIDIAN_WIKI_CN.md)).
