...
| `/implement`（每 ticket） | **Bind**：首 ticket 前 `--fingerprint-preflight`；每 ticket `/grill-adapter:wiki-materialize <ticket>`；Card 会先重验 remote-base/pack identity 并明确要求调用当前角色的 project skill；`source-truth-lint` hook；涌现候选经 journal skill 追加 |
| `/code-review` 后 | **Capture**：`/grill-adapter:update-wiki` 校验/折叠 journal，以最终证据 reconcile、显式归并 related claims、展示并应用 policy-compliant diff；skill pack 先由 `scaffold-practice-skill stage-card` 产生内容寻址候选，再与普通 Note 一样经 reviewed proposal/apply。确认精确发布 scope 后，把 applied receipts 按 repository 发布为可恢复的 draft PR，并恢复 base worktree（内含 grill 增量桥接的可选前置步）。开放 PR 仍 pending，merge + base 同步后才可发现 |
