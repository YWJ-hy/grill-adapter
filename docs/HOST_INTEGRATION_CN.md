...
| `/implement`（每 ticket） | **Bind**：首 ticket 前 `--fingerprint-preflight`；每 ticket `/grill-adapter:wiki-materialize <ticket>`；`source-truth-lint` hook；涌现候选经 journal skill 追加 |
| `/code-review` 后 | **Capture**：`/grill-adapter:update-wiki` 校验/折叠 journal，以最终证据 reconcile、显式归并 related claims、展示并应用 policy-compliant diff；确认精确发布 scope 后，把 applied receipts 按 repository 发布为可恢复的 draft PR，并恢复 base worktree（内含 grill 增量桥接的可选前置步） |
...
Claude Code 会直接注册 `agents/*.md` 的 3 个专用 agent。Codex 插件当前不注册该目录，因此 `wiki-research` 与 `lanhu-requirements` skill 会读取同一份自包含 agent prompt，并派生通用 sub-agent 执行。职责边界、输入与输出契约不变，只改变 dispatch 机制。
...
一次装齐 13 skills + 3 agents + 3 hooks + 2 MCP servers（legacy `shared-wiki` 与 Source-binding `obsidian-wiki`）。开发期不必安装：
