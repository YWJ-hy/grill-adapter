...
hook 命令路径写成 `__GRILL_ADAPTER_ROOT__/hooks/<hook>.sh`，install 时替换为 payload 绝对路径。hook 脚本自身经 `$(dirname "$0")/../scripts` 定位执行层脚本，无需二次改写。hook **无原生「当前 ticket」字段**——per-ticket 精度靠显式 `/wiki-materialize <ticket>`（首选），或 `GRILL_CURRENT_TICKET` env / `.adapter/current-ticket` marker（`wiki-reread.sh` 会读）。
...
- wiki 数据/绑定：`bootstrap-wiki` 播种 `.adapter/wiki/`；`.shared-adapter/settings.json` 声明 shared 绑定。
