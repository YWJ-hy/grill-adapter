...
grill-adapter 本身是一个 Claude Code 插件：skills、agents、hooks、shared-wiki MCP 全部随插件发货、一起激活。插件唯一做不到的事是改目标项目的 `CLAUDE.md`——所以一个「host 适配器」现在只剩一样东西：
...
一次装齐 12 skills + 3 agents + 4 hooks + 1 MCP server。开发期不必安装：
