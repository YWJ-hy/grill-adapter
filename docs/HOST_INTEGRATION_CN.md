...
| `/code-review` 后 | **Capture**：`/grill-adapter:update-wiki` 校验/折叠 journal，以最终证据 reconcile、显式归并 related claims、展示 policy-compliant diff，并记录 keep/skip/defer 与 candidate-to-write receipt（内含 grill 增量桥接的可选前置步） |
