...
plugin 一启用，12 个公开 skill、5 个 role agent、3 个 hook 和一个 Source-binding `obsidian-wiki` MCP server **一起注册、自动生效**——不往 `~/.claude/skills`、`~/.claude/agents` 拷文件，也不往你项目的 `.claude/settings.json` 里并 hook 片段。初始化 Obsidian 项目时使用 `setup-init-obsidian`，它会先检查 `grill-adapter` 与 `@grill-adapter/obsidian-wiki` 两个 npm 包，并复用它们的 CLI。
...
```bash
claude --plugin-dir "$PWD" plugin details grill-adapter   # 不安装即加载：应报 12 skills / 5 agents / 3 hooks / 1 MCP server
