...
install 末尾会**打印**一段 shared-wiki MCP 注册 JSON。只有你要用**跨 repo 共享 wiki** 才需要：按提示用 `claude mcp add-json -s user shared-wiki '...'` 注册一次（用户级）。server 启动读 `CLAUDE_PROJECT_DIR`，从每个项目的 `.shared-superpowers/settings.json` 自配置；没声明的项目自动 fail-closed。
...
`.superpowers/wiki/` 就绪（标准模板：backend / frontend / guides）。或在 Claude Code 里 `/init-wiki` 让 agent 基于项目盘点起草，`/import-wiki` + `/migrate-wiki` 导入并 section 化已有文档。
...
| 3 拆 ticket | `/to-tickets` | Disclose+Carry：`/wiki-research`（plan）→ 生成 `<plan>.wiki-context.json` | plan + `## Referenced Project Wiki` |
...
- **shared-wiki MCP 没连上？** 跑 `./manage.sh doctor <project>` 看绑定；确认 `.shared-superpowers/settings.json` 的 `wiki.sharedMcp.repoUrl` 有值；确认已用 `./manage.sh mcp-registration` 打印的 JSON 注册过一次。没声明就是 fail-closed（无 MCP shared wiki），属正常。
