...
plugin 一启用，12 个 skill、3 个 agent、4 个 hook 和两个 MCP server（legacy `shared-wiki`、Source-binding `obsidian-wiki`）**一起注册、自动生效**——不往 `~/.claude/skills`、`~/.claude/agents` 拷文件，也不往你项目的 `.claude/settings.json` 里并 hook 片段。
...
```bash
claude --plugin-dir "$PWD" plugin details grill-adapter   # 不安装即加载：应报 12 skills / 3 agents / 4 hooks / 2 MCP servers
