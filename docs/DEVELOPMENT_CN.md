...
- **wiki 授权 / 导入 / 导出 / 模板 / scaffold / 迁移规划**：`wiki-authorization-policy-smoke.sh`、`wiki-import-skill-path-smoke.sh`、`export-wiki-skills-smoke.sh`、`bootstrap-wiki-template-import.sh`、`init-wiki-inventory-smoke.sh`、`scaffold-practice-skill-smoke.sh`、`obsidian-wiki-migration-plan-smoke.sh`（source/target 快照、逐项决策、验收风险与 strength 推断确认门、binding fail-closed、确定性与零写入）。
