...
```bash
claude --plugin-dir "$PWD" plugin details grill-adapter   # 应报 11 skills / 1 agent / 3 hooks / 1 MCP server
...
- `release-check.sh` 是**发布前总门**，非破坏（plugin 用 `--plugin-dir` 只读加载，接线打在临时项目上，传入的 `<project-root>` 只被 `doctor` 只读使用）。它按序执行：
  ...
  6. **plugin 组件清单**：Claude 必须报满 11 skills / 1 agent / 3 hooks / 1 MCP；`tests/codex-plugin-smoke.sh` 必须通过 manifest 校验、隔离 marketplace 安装，并从 `codex debug prompt-input` 验证安装后模型可见的 11 个 skills。
