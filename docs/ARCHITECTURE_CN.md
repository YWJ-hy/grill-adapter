...
- **Maintenance Audit**：`obsidian_wiki_maintenance_summary` 只读聚合当前可读 binding 的 frontmatter metadata 与 canonical feature journal fold，输出 schema-v1 非权威 envelope。`/grill-adapter:wiki-maintenance` 只派生一个 `wiki-maintenance` role，以 summary 选出有界 stable identity batch，私下读取最多 24 个 bound Note body，判断 freshness、contradiction 与 overloaded Note，再返回 schema-v1 metadata-only report；coordinator 用 `wiki_maintenance_report.py` fail-closed 校验、原子落到 feature 本地工作目录并只暴露 compact summary。调用必须带规范 UTC 秒级 `asOf` 作为显式确定性 freshness clock；knowledge freshness、typed `contradicts` identity、repository/base health 与 candidate lifecycle 分组独立，`identityLimit` 对每类 stable identity 全局生效并带 `truncated` 标志，readable binding 另有 200 的结构硬上限。MCP 复用 `wiki_candidate_journal.py` 的完整 replay，不复制状态机；任何 journal 损坏、binding 不健康、correction identity 越界、dispatch/transport/lifecycle failure、report partial shape 或 binding drift 都走 `broken`。agent 禁止 write/apply/publish、journal/Git/task-context 修改；report 不含 Note body/summary、candidate claim/evidence/impact、outcome reason、journal transcript、agent reasoning、unbound identity、secret 或项目/Vault 路径，也绝不参与 task identity、readiness、Bind、proposal 或写授权。
