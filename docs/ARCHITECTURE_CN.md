...
- **模板、迁移与导出**：`wiki-template/`、`wiki-repo-skills/` + `wiki-repo-ci/`、`contracts/`。`wiki_migration_plan.py` fail-closed 产出 deterministic plan；`wiki_migration_apply.py` 只消费完全重验且无 conflict 的 plan，经 bridge 两阶段 CAS 与 receipt publisher 写 draft PR，manifest 支持中断恢复。verify 只认 merged + synchronized base，并通过正式 Obsidian CLI seam 证明映射/读取；cutover 另需确认、拒绝 active schema-v5 sidecar，只标记 legacy read-only archive。契约示例还包括 migration plan/manifest、wiki context/selection、ticket roster、candidate journal 与 publish run manifest。
