...
| **Bind** 角色化 task contract | 规划确认后生成同一批准快照的 `<taskId>.wiki-implement.md` 与 `<taskId>.wiki-review.md`；`/grill-adapter:wiki-materialize <ticket>` 校验并消费对应 Markdown，`wiki-readiness` 在 receipt 中绑定文件 digest | implement 逐 task + code-review 两轴前 |
