...
`wiki-readiness` 内部复用 `scripts/wiki_materialize_task.py`——规划 freeze/late Carry 时只从绑定的 Obsidian Source 取数，生成角色化 Markdown，含**有界 1 跳 `depends-on` 闭包**；执行期只验证并消费冻结文件。`wiki-materialize` 仅保留为显式恢复/诊断工具，不是 host workflow 触点。
