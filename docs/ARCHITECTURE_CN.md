...
- **模板、迁移规划与导出**：`wiki-template/`、`wiki-repo-skills/` + `wiki-repo-ci/`、`contracts/`。`wiki_migration_plan.py` 只读 legacy project/shared roots 与 bound Source snapshots，把 page/section/index/graph/legacy Skill Card 逐项映射成 deterministic plan；semantic/identity/edge/pack/neutrality/navigation 风险只进 confirmation gate，不执行 apply。契约示例还包括 wiki context/selection、ticket roster、candidate journal 与 publish run manifest。
