...
- **Maintenance Audit + Consolidation**：`obsidian_wiki_maintenance_summary` 只读聚合当前可读 binding 的 frontmatter metadata 与 canonical journal lifecycle；`obsidian_wiki_consolidation_candidates` 则复用同一完整 journal fold，向 maintenance child 提供全局有界、规范化的 unresolved candidate 语义输入与 journal/candidate digest。`/grill-adapter:wiki-maintenance` 两种模式始终只派生同一个 `wiki-maintenance` role：audit 私下读取最多 24 个 bound Note body，判断 freshness/contradiction/overloaded Note；consolidation 私下识别等价 claim、矛盾 claim 与必须保持独立的不同 trigger/lifecycle/failure mode/validation path contract。两者都只返回 schema-v1 metadata report；coordinator 分别用 `wiki_maintenance_report.py` / `wiki_maintenance_consolidation_report.py` fail-closed 校验并原子落盘，只暴露 compact summary。consolidation validator 在写入前重放当前 canonical journals；同一 candidate 可在 equivalent 与 contradictory 关系各出现一次，但同关系重复、group/list 重叠、分类遗漏、journal drift、单成员 group、正文泄漏、截断或 evidence gap 伪装成功都走 `broken`，并保留旧 report 供中断恢复。agent 禁止 write/apply/publish、journal/Git/task-context 修改；report 不含 Note/candidate body、claim/evidence/impact、outcome reason、journal transcript、agent reasoning、secret 或项目/Vault 路径，也绝不参与 task identity、readiness、Bind、Capture 决策、proposal 或写授权。
...
- **obsidian-wiki MCP + write/publish bundle `mcp/obsidian-wiki/`**：同一提交型 bundle 暴露绑定只读工具、metadata-only maintenance summary、bounded private consolidation candidate input、Note proposal/apply MCP/JSON CLI、可恢复 `publish` JSON CLI，以及独立 `serve-write-bridge` 入口。bundle 同时携带 summary/consolidation 所需的 canonical journal reader。bridge 只监听 loopback，以 token 鉴权并对 bound Source 做 policy/neutrality/CAS 校验。publisher 只消费 journal 中 `kept+applied` receipts，按 `repositoryRef` 锁仓并在锁内重验 base/remote/path/hash；commit 前用 Git staged-tree object ID 保存可恢复身份，创建 allowlist commit + draft PR、协调 peer PR，并把 worktree 恢复到 clean base。run manifest 留在项目 `.grill-adapter/context/`，开放 PR 不进入 formal read。
