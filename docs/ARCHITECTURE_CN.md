...
- **Maintenance Audit + Consolidation**：同一个 `wiki-maintenance` role 支持 proposal-only audit/candidate consolidation，以及只由 `update-wiki review/publish` 调用的 Outbox consolidation。后者私下读取 queued draft，等价 claim 通过受限 correction interface 合并，矛盾/证据不足追加 deferred successor，独立 trigger/lifecycle/failure mode/validation path/task routing/Card/ADR contract 保持分离；它不发布、不改 formal Note、不选择真相。其余两种模式仍只返回经 validator 校验的 schema-v1 metadata report。
