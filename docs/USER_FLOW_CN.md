...
| 5 | `/code-review` 后 | Capture | `/grill-adapter:update-wiki`（内部先把 grill 增量转成 candidate events） | journal outcome receipt + 后续 Wiki proposed change |
...
4. 每条候选经 `/candidate-journal outcome` 追加 `kept` / `skipped` / `deferred`。kept/skipped 是终态，deferred 后可继续决策；journal 保留为中断恢复 receipt，不删除、不提交。

5. `wiki-capture-suggest` hook（Stop）只在 pending/deferred 时提醒，journal 全终态静默；invalid journal 单独报错，阻止静默漏 Capture。
