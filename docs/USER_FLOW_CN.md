...
3. 对每个 pending/deferred 候选逐条做 keep-or-skip：
...
5. 每条候选经 `/candidate-journal outcome` 追加 `kept` / `skipped` / `deferred`。只有 apply 返回匹配的 post-write identity 才能记 `kept`；kept/skipped 是终态，deferred 后可继续决策。journal 保留为中断恢复 receipt，不删除、不提交。此时 Note 只是待后续 PR publishing 的 staged knowledge state，不代表已合并或已进入正式检索。
