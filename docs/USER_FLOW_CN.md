...
| 3 | `/to-tickets`（规划） | Disclose + Carry | `/wiki-research`（phase plan）+ `wiki_context_render.py --scaffold` → 建 ticket roster → `--finalize` + `/source-truth-check`（plan-pre / plan-review） | `.adapter/context/<feature-slug>.` 下的 `wiki-selection.json`、`wiki-context.json`、`ticket-roster.json` |
