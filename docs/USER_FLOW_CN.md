...
| 触点 | 含义 | 载体 |
| --- | --- | --- |
| **Disclose（披露）** | 规划前轻量披露相关 wiki 作上下文 | `/wiki-research` |
| **Carry（携带）** | 把选中的 wiki 固化进计划期 sidecar | `.wiki-context.json`（`wiki_context_render.py --scaffold` → `--finalize`） |
| **Bind（绑定）** | 执行每个 ticket 前 reread 硬约束整段 | `/wiki-materialize <ticket>`（跑 `wiki_materialize_task.py`，含 1 跳 depends-on 闭包）+ hook 兜底 |
| **Capture（捕获）** | review 后把新沉淀的知识回写 wiki | `/update-wiki`（由 `grill_context_to_candidates.py` 喂候选行） |
...
| --- | --- | --- | --- | --- |
| 0 | （可选）蓝湖录入 | 输入准备 | `/lanhu-requirements <link> frontend\|backend` | `.lanhu/.../index.md` 证据包 |
| 1 | `/grill-with-docs`（质询/发现） | Disclose | `/wiki-research`（phase brainstorm） | 轻量上下文（**不写 sidecar**） |
| 2 | `/to-spec` | Verify | `/source-truth-check`（render spec-pre） | 真实源校验结果 |
| 3 | `/to-tickets`（规划） | Disclose + Carry | `/wiki-research`（phase plan）+ `wiki_context_render.py --scaffold/--finalize` + `/source-truth-check`（plan-pre / plan-review） | `.wiki-selection.json`、`.wiki-context.json`、plan 的 `## Referenced Project Wiki` |
| 4 | `/implement`（每 ticket） | Bind | `/wiki-materialize <ticket>`（首个 ticket 前 `--fingerprint-preflight`）+ `source-truth-lint` hook | reread 的硬约束整段、lint 结果、`.wiki-candidates.jsonl` 追加 |
| 5 | `/code-review` 后 | Capture | `grill_context_to_candidates.py` → `/update-wiki` | 更新后的 wiki 页 / 节 |
| 6 | `/diagnosing-bugs` | Disclose + Capture | `/wiki-research`（phase debug）→ `/break-loop` → `/update-wiki` | 根因复盘 + wiki 回写 |
...
2. 用 render 脚本把选择 scaffold 成 sidecar，编辑每节归属，再 finalize：

   ```bash
   wiki_context_render.py --scaffold    # 由 .wiki-selection.json 生成 .wiki-context.json
   # 人工编辑每节的 destination（一次）
   wiki_context_render.py --finalize    # 固化 sidecar
   ```

3. 在 plan 中加入 `## Referenced Project Wiki` 小节，登记本计划引用的 wiki。
...
- spec / plan 是否提交遵循 **gitignore-aware commit 政策**：由 `.gitignore` 决定是否入库，不额外加开关、不 `git add -f`。
...
review 通过后，把本轮新沉淀的知识回写 wiki。

1. 把 grill 的 CONTEXT.md / ADR 增量转成候选行：

   ```bash
   grill_context_to_candidates.py       # → .wiki-candidates.jsonl
   ```

2. 消费候选，逐条做 keep-or-skip：

   ```
   /update-wiki
   ```

   `/update-wiki` 对每条候选逐一过闸：**durable 闸 → sectionize（分节）→ type（定类型）→ `[[page#section]]` 边 → dedup（去重）→ 中性化 → 授权**，最终只保留真正值得沉淀的知识。
...
- **Verify**：`/source-truth-check` 在 `/to-spec`（spec-pre）与 `/to-tickets`（plan-pre / plan-review）处 render，把关 spec / plan 的事实基础。
...
- 绑定声明写在项目的 `.shared-superpowers/settings.json` 的 `wiki.sharedMcp` 里，声明本项目连哪个 shared wiki。
...
- **Bind 靠约定 + hook**：`/wiki-materialize <ticket>` 是按约定逐 ticket 调用的；`wiki-reread.sh` hook（UserPromptSubmit / SessionStart）只是**粗粒度兜底**——因为 hook 拿不到 ticket 字段，无法精确到单个 ticket。所以 hook 是 backstop，不是替代。
- **中途 capture 退回末尾一次**：执行中途无法逐条即时回写，durable 候选先攒进 `.wiki-candidates.jsonl`，到步骤 5 末尾统一由 `/update-wiki` 消费一次。
...
**Skills（安装到 `~/.claude/skills`）**：`wiki-research`、`wiki-materialize`、`update-wiki`、`init-wiki`、`import-wiki`、`migrate-wiki`、`publish-shared-wiki`、`shared-wiki-mcp`、`scaffold-practice-skill`、`lanhu-requirements`、`break-loop`、`source-truth-check`。
...
**Agents（`~/.claude/agents`）**：`wiki-researcher`、`lanhu-frontend-requirements-analyst`、`lanhu-backend-requirements-analyst`。

**载荷根 `__GRILL_ADAPTER_ROOT__`（默认 `~/.claude/grill-adapter`）**：`scripts/`、`contracts/`、`hooks/`、`mcp/`。
