...
| 4 | `/implement`（每 ticket） | Bind | 首个 ticket 前 `--fingerprint-preflight`；每个 v5/v6 sidecar 都运行 `/grill-adapter:wiki-materialize <ticket>` | 当前任务的权威硬约束整段/Note、角色所需 Skill Card、1 跳闭包、lint 结果、`.wiki-candidates.jsonl` 追加 |
