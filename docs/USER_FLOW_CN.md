...
| **Bind（绑定）** | schema-v5 sidecar 在每个 ticket 前 reread 硬约束整段；schema-v6 的权威 stable-ID Note reread 在后续 Bind 切片提供 | `/grill-adapter:wiki-materialize <ticket>`（v5：跑 `wiki_materialize_task.py`，含 1 跳 depends-on 闭包）+ hook 兜底 |
...
| 4 | `/implement`（每 ticket） | Bind | 首个 ticket 前 `--fingerprint-preflight`；现有 v5 sidecar 才可 `/grill-adapter:wiki-materialize <ticket>`，v6 等待 stable-ID Bind | v5 reread 的硬约束整段、lint 结果、`.wiki-candidates.jsonl` 追加 |
...
- 此阶段**只披露、不写 sidecar**——不产生 selection 或 `.wiki-context.json`。
...
已有 schema-v5 sidecar 的执行阶段以 ticket 为单位，逐个 **reread 硬约束整段**。schema-v6 目前仅完成 Carry：在权威 stable-ID Note reread 落地前，它的摘要不能替代整段约束。
