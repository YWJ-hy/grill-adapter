...
3. `obsidian_wiki_apply_note_change` 把同一输入交给 bridge。bridge 再独立校验 Bearer token、Source manifest policy、identity/typed links、允许 root、`_meta` 禁写与 Shared neutrality，并在写入瞬间重查 expected hash；冲突返回 409，不覆盖并发修改。
