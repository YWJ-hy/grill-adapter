...
- 入口：`/lanhu-requirements <蓝湖链接> frontend|backend <可选需求命名>`。角色形式接受 `frontend`/`front-end`/`fe`/`前端` 与 `backend`/`back-end`/`be`/`后端`；缺省时读 `.adapter/settings.json` 的 `lanhu.role`，仍缺失或歧义时向用户询问，角色未定不调用任何分析师或 Lanhu MCP。
