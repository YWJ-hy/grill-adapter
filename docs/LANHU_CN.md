...
- 入口：`/grill-adapter:lanhu-requirements <蓝湖链接> frontend|backend <可选需求命名>`。角色形式接受 `frontend`/`front-end`/`fe`/`前端` 与 `backend`/`back-end`/`be`/`后端`；缺省时读 `.adapter/settings.json` 的 `lanhu.role`，仍缺失或歧义时向用户询问，角色未定不调用任何分析师或 Lanhu MCP。
- 角色分析师：前端路由到 `grill-adapter:lanhu-frontend-requirements-analyst`，后端路由到 `grill-adapter:lanhu-backend-requirements-analyst`，两者共享同一份基础来源（`role-prd/analyst.common.md`，与其共同输入 `role-prd/{frontend,backend}.md` 同级；**不能放 `agents/`**，插件会把那里的 `.md` 全部注册成 agent）。
- 模板：`role-prd/frontend.md`、`role-prd/backend.md` 定义各角色的固定包结构与职责边界。
- 产物：`.lanhu/MM-DD-需求名称/` 包目录，以 `index.md` 为唯一入口。

## 录入工作流与取数边界

- **角色优先**：先从 `.adapter/settings.json` 的 `lanhu.role` 解析，其次从用户输入解析；仍缺失 / 歧义 / 前后端都要时先问用户选一个，角色未定前不派发分析师、不调 Lanhu MCP。
...
- 不写 `.adapter/wiki/`，不写 spec、ticket，也不写 `.adapter/context/` 下的任何东西（wiki-context sidecar、ticket roster、candidates）。
...
1. 用户给出蓝湖链接时，先跑 `/grill-adapter:lanhu-requirements <链接> frontend|backend`，选定角色并做 URL-rooted 页面选择。
