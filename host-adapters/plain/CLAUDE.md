...
For `ready`, `wiki-readiness` internally runs `/grill-adapter:wiki-materialize <task-id>` with role `implementer` and consumes the frozen `<task-id>.wiki-implement.md` task contract. Do not invoke a second materialization just because the SessionStart hook noticed a pending snapshot. The matching `<task-id>.wiki-review.md` is frozen at the same approval point for later review. The files contain only the routed hard Notes, role-required Skill Cards, and bounded de-duplicated 1-hop `depends_on` closure for their roles; embedded metadata and the receipt digest protect the task fingerprint and body. The `source-truth-lint` hook lints real changed files; resolve any `block`/`ask` before completing the task.
...
A valid `ready` receipt reuses only the current task's `<task-id>.wiki-review.md`, containing reviewer-routed hard constraints, direct `depends_on` closure, and reviewer-required Skill Cards. Give both axes the **same read-only handoff Markdown file** and require each to read it; the actual reviewer invokes every verified project skill required by a Card. Standards and Spec retain their separate responsibilities and output structure.
