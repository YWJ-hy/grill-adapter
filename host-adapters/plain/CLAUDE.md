...
4. After the user approves the routed constraints, run `/grill-adapter:wiki-readiness` freeze for each roster task to create `<taskId>.wiki-implement.md` and `<taskId>.wiki-review.md` from the same approved snapshot.
...
For `ready`, the skill runs `/grill-adapter:wiki-materialize <task-id>` with role `implementer` and consumes the frozen `<task-id>.wiki-implement.md` task contract. The matching `<task-id>.wiki-review.md` is frozen at the same approval point for later review. The files contain only the routed hard Notes, role-required Skill Cards, and bounded de-duplicated 1-hop `depends_on` closure for their roles; embedded metadata and the receipt digest protect the task fingerprint and body. The `source-truth-lint` hook lints real changed files; resolve any `block`/`ask` before completing the task.
