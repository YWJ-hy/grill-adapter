...
For a Wiki lifecycle audit, invoke `$grill-adapter:wiki-maintenance audit <feature-slug>`. It dispatches exactly one read-only maintenance agent, immediately waits on that same path, and lets only that role call `obsidian_wiki_maintenance_summary` with a normalized UTC `asOf` timestamp and bounded identity/body-read limits. The agent privately audits freshness, contradiction, and overloaded Notes; the main session receives only a validated schema-v1 report path plus compact summary. The report is metadata-only, proposal-only, non-authoritative, and never replaces task identity, readiness, Bind, proposal, or authorization. Dispatch/transport/capacity/lifecycle failure, malformed journals, unhealthy bindings, identity drift, or report validation failure is `broken`, never an empty successful audit; do not search inline or reuse a stale report. The role cannot call write/apply/publish or modify journal, Git, task context, or host state. Invoke `$grill-adapter:migrate-wiki` separately for its bounded legacy section-repartition pass or confirmed Obsidian Note maintenance/repartition mode; those modes remain proposal-driven and never edit cut-over legacy roots directly.
