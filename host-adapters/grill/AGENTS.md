...
Before a Wiki lifecycle audit, call read-only `obsidian_wiki_maintenance_summary` with a normalized UTC `asOf` timestamp and a bounded global-per-category identity limit; reuse the timestamp for a repeated inventory. Its metadata-only envelope separates freshness/contradiction, repository/base health, and candidate lifecycle; it is non-authoritative and never replaces task identity, readiness, Bind, proposal, or authorization. Malformed journals, unhealthy bindings, and unbound correction identities fail closed. Then invoke `$grill-adapter:migrate-wiki` explicitly: run its bounded legacy section-repartition pass before migration when existing markers may be too broad, or its Obsidian Note maintenance/repartition mode after migration for a confirmed batch audit. Both modes are proposal-driven and never edit cut-over legacy roots directly.
