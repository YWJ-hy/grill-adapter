---
name: wiki-research
description: Disclose relevant project/shared wiki context and carry it into approved task contracts. Requires explicit invocation or project opt-in; standalone grill remains inert.
---
...
- Execution-time consumption is **not** this skill — the internal Bind reader owned by `wiki-readiness` reads the frozen role-specific task Markdown. The freeze command is the only planning-time step that materializes both roles; a missing pair is created as an explicit implementation-entry fallback by `wiki-readiness`.
