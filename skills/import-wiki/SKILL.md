...
Use this skill when the user provides an existing wiki page or directory that should be copied into this adapter's `.adapter/wiki` or `.shared-adapter/wiki` tree.
...
For shared imports, also honor `.shared-adapter/settings.json` `wiki.sharedNeutrality.blockedTerms` and `wiki.sharedNeutrality.blockedPatterns`. If the source contains configured system identifiers, refuse the shared import and ask the user to neutralize it first or import it into project wiki instead.
...
Use `import-wiki` for one-time structural migration of existing user wiki pages into `.adapter/wiki/` or `.shared-adapter/wiki/`.
