---
name: import-wiki
description: "Structurally import a user-provided wiki page or directory into project or shared wiki without overwriting content. Use only when explicitly invoked or the project is wired/configured for grill-adapter; do not auto-use in a standalone grill workflow."
---
...
Use this skill when the user provides an existing wiki page or directory that should be copied into this adapter's `.grill-adapter/wiki` or `.shared-adapter/wiki` tree.

The goal is structural migration: preserve source content, place it under the selected wiki root, avoid overwriting existing different content, and rebuild indexes. This skill does not semantically merge imported rules into project-owned or shared-owned wiki pages.
...
- Before importing, honor the selected root's `wiki.roots.<project|shared>.updateAuthorization.createNewDocument` policy from `.grill-adapter/settings.json`.
- If `.grill-adapter/settings.json` records the selected root in `wiki.legacyRuntime.roots` with `mode: read-only-archive`, stop. Explicit authorization never reopens a cut-over legacy root, and `wiki_import.py` enforces this mechanically.
...
Structural import creates wiki documents, so it is governed by the selected root's `wiki.roots.<project|shared>.updateAuthorization.createNewDocument` setting:

- `.grill-adapter/settings.json` controls both legacy roots; use `wiki.roots.project` for `.grill-adapter/wiki/` and `wiki.roots.shared` for `.shared-adapter/wiki/`.
...
For shared imports, also honor `.grill-adapter/settings.json` `wiki.roots.shared.sharedNeutrality.blockedTerms` and `wiki.roots.shared.sharedNeutrality.blockedPatterns`. If the source contains configured system identifiers, refuse the shared import and ask the user to neutralize it first or import it into project wiki instead.
...
Use `import-wiki` for one-time structural migration of existing user wiki pages into `.grill-adapter/wiki/` or `.shared-adapter/wiki/`.
