...
`wiki_materialize_task.py` is the **single fixed fetcher**. It calls only the bound Obsidian MCP `read-notes` and `graph-neighbors` CLI endpoints: it checks every direct Note/Card's source, role, path, stable ID, type, binding digest, content hash, summary, and Skill Card name/version/contract-hash/trigger/role identity against the carried sidecar before emitting content. ADR projections also re-resolve the carried project-relative authority path under the current project root and compare both the path-derived source ID and current content hash; missing, escaping, malformed, or changed ADRs fail Wiki validation before any projection text is emitted. It closes only the selected hard constraints' **bounded, de-duplicated 1-hop `depends-on` set** and does not follow a dependency's dependencies.
