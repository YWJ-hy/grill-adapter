---
name: break-loop
description: Use after the host debugging workflow (e.g. grill diagnosing-bugs) or an equivalent verified bug fix to analyze why the bug happened, how to prevent recurrence, and whether durable findings should be handed to update-wiki. Use only when explicitly invoked or the project is wired/configured for grill-adapter; do not auto-use in a standalone grill workflow.
---
...
This is an after-action review. It does not replace the host debugging workflow (grill's `diagnosing-bugs`, or an equivalent). If the bug is not fixed, the root cause is unclear, or verification is incomplete, stop this retrospective and return to the debugging workflow first.
...
```text
diagnosing-bugs (or an equivalent debugging workflow)
→ break-loop
...
If any item is missing, report what is missing and use the debugging workflow before continuing.
