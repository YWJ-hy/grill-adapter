...
Use this skill to help the user initialize legacy `.adapter/wiki/` and/or `.shared-adapter/wiki/` roots from the current project state.
...
- If `.shared-adapter/settings.json` sets `wiki.provider: obsidian`, stop without writing legacy Wiki content. Bound Obsidian Sources are the active runtime; legacy roots are only migration evidence and never a fallback.
...
2. Inspect `.shared-adapter/settings.json`. If it sets `wiki.provider: obsidian`, stop without writing legacy Wiki content and direct the user to the bound Source workflow in `docs/OBSIDIAN_WIKI_CN.md`. Otherwise ensure at least one wiki index exists: `.adapter/wiki/index.md` or `.shared-adapter/wiki/index.md`; if neither exists, ask the user to run the legacy-only `bootstrap-wiki` first with `--wiki-root project` or `--wiki-root shared`.
...
- `projectRoot`
- `wikiRoot` (compatibility field for `.adapter/wiki/`)
- `wikiRoots`
- `focusHint`
- `languages`
- `stackSignals`
...
- If inventory signals include current-system identifiers, write them only to `.adapter/wiki/` or replace them with neutral terms before writing shared wiki.
- Use `.adapter/wiki/` for project-specific business, integration, deployment, or local override knowledge.
