...
def resolve_wiki_file(raw: str, wiki_root: Path, project_root: Path) -> Path | None:
    ...
    copies is usually project-root-relative (``.grill-adapter/wiki/foo/bar.md``). Joining that
    onto the wiki root double-prefixes it into ``.grill-adapter/wiki/.grill-adapter/wiki/…`` →
